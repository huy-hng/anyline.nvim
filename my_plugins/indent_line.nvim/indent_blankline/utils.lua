...
M.error_handler = function(err, level)
	if err:match('Invalid buffer id.*') then return end
	if not pcall(require, 'notify') then err = string.format('indent-blankline: %s', err) end
...
M.is_indent_blankline_enabled = M.memo(
	function(
		...
		if b_enabled ~= nil then return b_enabled end
		if g_enabled ~= true then return false end
		if disable_with_nolist and not opt_list then return false end
		...
		for _, ft in ipairs(filetype_exclude) do
			for _, undotted_filetype in ipairs(undotted_filetypes) do
				if undotted_filetype == ft then return false end
			end
		end

		for _, bt in ipairs(buftype_exclude) do
			if bt == buftype then return false end
		end

		for _, bn in ipairs(bufname_exclude) do
			if vim.fn['matchstr'](bufname, bn) == bufname then return false end
		end
...
M.get_current_context = function(type_patterns, use_treesitter_scope)
	...
	if not ts_utils_status then
		vim.schedule_wrap(
			function()
				M.error_handler(
					'nvim-treesitter not found. Context will not work',
					vim.log.levels.WARN
				)
			end
		)()
	...
	while cursor_node do
		...
		for _, rgx in ipairs(type_patterns) do
			if node_type:find(rgx) then
				...
				if node_start ~= node_end then return true, node_start + 1, node_end + 1, rgx end
			end
		end
...
M.get_variable = function(key)
	if vim.b[key] ~= nil then return vim.b[key] end
	if vim.t[key] ~= nil then return vim.t[key] end
	return vim.g[key]
end
...
M.merge_ranges = function(ranges)
	...
	for i = 2, #ranges do
		...
		if current_end >= next_start - 1 then
			if current_end < next_end then merged_ranges[#merged_ranges][2] = next_end end
		else
...
M.binary_search_ranges = function(ranges, target_range)
	...
	if not exact_match then idx_mid = idx_start end
