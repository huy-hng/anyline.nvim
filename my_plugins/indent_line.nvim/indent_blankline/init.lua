...
M.setup = function(options)
	...
	vim.g.indent_blankline_char =
		o(options.char, vim.g.indent_blankline_char, vim.g.indentLine_char, '│')
	vim.g.indent_blankline_char_blankline =
		o(options.char_blankline, vim.g.indent_blankline_char_blankline)
	...
	vim.g.indent_blankline_indent_level =
		o(options.indent_level, vim.g.indent_blankline_indent_level, 20)
	...
	vim.g.indent_blankline_viewport_buffer =
		o(options.viewport_buffer, vim.g.indent_blankline_viewport_buffer, 10)
	vim.g.indent_blankline_use_treesitter =
		o(options.use_treesitter, vim.g.indent_blankline_use_treesitter, false)
	...
	vim.g.indent_blankline_show_foldtext =
		o(options.show_foldtext, vim.g.indent_blankline_show_foldtext, true)
	...
	vim.g.indent_blankline_strict_tabs =
		o(options.strict_tabs, vim.g.indent_blankline_strict_tabs, false)
	...
	vim.g.indent_blankline_char_priority =
		o(options.char_priority, vim.g.indent_blankline_char_priority, 1)
...
local refresh = function(scroll)
	...
	then
		if vim.b.__indent_blankline_active then vim.schedule_wrap(utils.clear_buf_indent)(bufnr) end
	...
	local range = math.min(
		win_end + v('indent_blankline_viewport_buffer'),
		vim.api.nvim_buf_line_count(bufnr)
	)

	if not vim.b.__indent_blankline_ranges then vim.b.__indent_blankline_ranges = {} end
	...
	if scroll then
		...
		if vim.b.__indent_blankline_ranges[left_offset_s] then
			...
			local idx_candidate =
				utils.binary_search_ranges(blankline_ranges, { win_start, win_end })
			...
			if not need_to_update then return end
	...
	local space_char_blankline_highlight_list = v(
		'indent_blankline_space_char_blankline_highlight_list'
	) or {}
	...
	if v('indent_blankline_use_treesitter') then
		...
		if not ts_query_status then
			vim.schedule_wrap(
				function()
					utils.error_handler(
						'nvim-treesitter not found. Treesitter indent will not work',
						vim.log.levels.WARN
					)
				end
			)()
		...
		use_ts_indent = ts_query_status
			and ts_indent_status
			and ts_query.has_indents(vim.bo.filetype)
	end
	...
	local shiftwidth =
		utils._if(tabs, utils._if(no_tab_character, 2, vim.bo.tabstop), vim.bo.shiftwidth)
	...
	local show_current_context_start_on_current_line =
		v('indent_blankline_show_current_context_start_on_current_line')
	...
	local get_virtual_text = function(
		...
		local indent_char_list =
			utils._if(blankline and #char_list_blankline > 0, char_list_blankline, char_list)
			...
			if space_count > 0 then
				...
				if show_indent_or_eol_char then
					if table.maxn(virtual_string) >= index + space_count then
						...
						tmp_string =
							table.concat(virtual_string, '', index + 1, index + space_count)
					end
				else
					if table.maxn(virtual_string) >= index + space_count - 1 then
						tmp_string =
							table.concat(virtual_string, '', index, index + space_count - 1)
					end
				end
				table.insert(virtual_text, {
					utils._if(
	...
	for i = 1, #lines do
		local async = vim.loop.new_async(function()
			...
			if not prev_indent
				or indent + utils._if(extra, 1, 0) <= prev_indent + max_indent_increase
			then
				prev_indent = indent
			end
...
M.refresh = function(scroll) xpcall(refresh, utils.error_handler, scroll) end
