...
local function get_current_context(bufnr)
	...
	for _, line_pair in ipairs(ranges) do
		...
		if cursor_line >= start and cursor_line <= stop then --
			return { start = start, stop = stop, column = column }
		end
	end
end
...
function ContextManager:new(bufnr, context_opts)
	...
	new.bufnr = bufnr or vim.api.nvim_get_current_buf()
...
function ContextManager:update_buffer(data)
	...
	context_data.column = context_data.column - lines.get_scroll_offset()
...
function ContextManager:changed_context(new_context)
	...
	local start = c.start ~= new_context.start
	local stop = c.stop ~= new_context.stop
	local column = c.column ~= new_context.column

	if start and stop and column then return true end
end
